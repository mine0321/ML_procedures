...
class Sampling(object):
    ...
    def run_sampling(self):
        ...
        push_df = self.sampling(
            acce_df, self.min_search_time, self.max_search_time,
    ...
    def sampling(
        self, acce_df, min_search_time, max_search_time,
        max_first_push_time, min_last_push_time, min_sample_time,
        max_sample_time, finish, timing
    ):

        push = self.find_initialpush(
            acce_df, self.acce_col, max_first_push_time)
        finish_time = finish - min_last_push_time

        pushs = self.find_pushs_acce(
            push, acce_df, self.acce_col, finish_time,
    ...
    def find_pushs_acce(self, push, df, col, finish, min_time, max_time):
        pushs = [push]
        while pushs[-1] < finish:
            search_range = [
                float(push + min_time), float(push + max_time)]
            ind = df[col][(search_range[0] <= df.time) & (
                df.time <= search_range[1])].idxmax()
            ind = self.find_larger(df, ind, col)
            push = self.check_push(ind, df, col, push)
            if push is None:
                push = df.time[ind]
                pushs.append(push)
        return pushs

    def check_push(self, ind, df, col, push):
        search_range = [
            float(df.time[ind]) - self.search_time, float(df.time[ind])]
        pre_ind = df[col][(search_range[0] <= df.time) & (
            df.time <= search_range[1])].idxmax()
        maxval = df[col][
            (float(push) <= df.time) & (df.time <= df.time[ind])].max()
        minval = df[col][
            (float(push) <= df.time) & (df.time <= df.time[ind])].min()
        p_p = maxval - minval
        if (pre_ind == ind) & (p_p > self.check_p_p):
            return None
        else:
            return df.time[ind]

    def find_initialpush(self, df, col, max_time):
        ind = df[col][(0 <= df.time) & (df.time <= max_time)].idxmax()
        ind = self.find_larger(df, ind, col)
        return df.ix[ind].time

    def find_larger(self, df, ind, col):
        pre_ind = None
        while pre_ind != ind:
            pre_ind = ind
            search_range = [
                float(df.time[ind]), float(df.time[ind]) + self.search_time]
            ind = df[col][(search_range[0] <= df.time) & (
                df.time <= search_range[1])].idxmax()
        return ind
