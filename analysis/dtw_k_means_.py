...
def _labels_inertia_precompute_dense(X, x_squared_norms, centers, distances):
    ...
    #                                     squared=True)
    all_distances = dtw_distances(centers, X, x_squared_norms,
                                  squared=True)
...
def dtw_distances(centers, X, x_squared_norms, squared=False):
    import mlpy
    distances = []
    for c in centers:
        distances.append(np.array(map(lambda x: mlpy.dtw_std(x, c), X)))
